...
export const metadata: Metadata = {
  title: "Kids Article Details — Learn Armenian in English",
  description: "Read our fun Armenian articles for kids."
};

const SingleKidsBlogPage = async ({ params }: { params: Promise<{ id: string }> }) => {
  const { id } = await params;
  let post: Blog | null = null;

  try {
    const response = await axiosInstance.get(`/kids-articles/${id}`);
    post = response.data.data;
  } catch (error) {
    console.error("Error fetching kids article:", error);
  }
...
export default SingleKidsBlogPage;
